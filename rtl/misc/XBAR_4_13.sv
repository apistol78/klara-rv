// Do NOT edit; this file is autogenerated.
`default_nettype none

// #note master 0 has highest priority.
module XBAR_4_13(
	input wire i_reset,
	input wire i_clock,

	// Master 0
	input wire i_m0_rw,
	input wire i_m0_request,
	output bit o_m0_ready,
	input wire [31:0] i_m0_address,
	output bit [31:0] o_m0_rdata,
	input wire [31:0] i_m0_wdata,
	input wire [3:0] i_m0_wmask,

	// Master 1
	input wire i_m1_rw,
	input wire i_m1_request,
	output bit o_m1_ready,
	input wire [31:0] i_m1_address,
	output bit [31:0] o_m1_rdata,
	input wire [31:0] i_m1_wdata,
	input wire [3:0] i_m1_wmask,

	// Master 2
	input wire i_m2_rw,
	input wire i_m2_request,
	output bit o_m2_ready,
	input wire [31:0] i_m2_address,
	output bit [31:0] o_m2_rdata,
	input wire [31:0] i_m2_wdata,
	input wire [3:0] i_m2_wmask,

	// Master 3
	input wire i_m3_rw,
	input wire i_m3_request,
	output bit o_m3_ready,
	input wire [31:0] i_m3_address,
	output bit [31:0] o_m3_rdata,
	input wire [31:0] i_m3_wdata,
	input wire [3:0] i_m3_wmask,

	// Slave 0
	output bit o_s0_rw,
	output bit o_s0_request,
	input wire i_s0_ready,
	output bit [31:0] o_s0_address,
	input wire [31:0] i_s0_rdata,
	output bit [31:0] o_s0_wdata,
	output bit [3:0] o_s0_wmask,

	// Slave 1
	output bit o_s1_rw,
	output bit o_s1_request,
	input wire i_s1_ready,
	output bit [31:0] o_s1_address,
	input wire [31:0] i_s1_rdata,
	output bit [31:0] o_s1_wdata,
	output bit [3:0] o_s1_wmask,

	// Slave 2
	output bit o_s2_rw,
	output bit o_s2_request,
	input wire i_s2_ready,
	output bit [31:0] o_s2_address,
	input wire [31:0] i_s2_rdata,
	output bit [31:0] o_s2_wdata,
	output bit [3:0] o_s2_wmask,

	// Slave 3
	output bit o_s3_rw,
	output bit o_s3_request,
	input wire i_s3_ready,
	output bit [31:0] o_s3_address,
	input wire [31:0] i_s3_rdata,
	output bit [31:0] o_s3_wdata,
	output bit [3:0] o_s3_wmask,

	// Slave 4
	output bit o_s4_rw,
	output bit o_s4_request,
	input wire i_s4_ready,
	output bit [31:0] o_s4_address,
	input wire [31:0] i_s4_rdata,
	output bit [31:0] o_s4_wdata,
	output bit [3:0] o_s4_wmask,

	// Slave 5
	output bit o_s5_rw,
	output bit o_s5_request,
	input wire i_s5_ready,
	output bit [31:0] o_s5_address,
	input wire [31:0] i_s5_rdata,
	output bit [31:0] o_s5_wdata,
	output bit [3:0] o_s5_wmask,

	// Slave 6
	output bit o_s6_rw,
	output bit o_s6_request,
	input wire i_s6_ready,
	output bit [31:0] o_s6_address,
	input wire [31:0] i_s6_rdata,
	output bit [31:0] o_s6_wdata,
	output bit [3:0] o_s6_wmask,

	// Slave 7
	output bit o_s7_rw,
	output bit o_s7_request,
	input wire i_s7_ready,
	output bit [31:0] o_s7_address,
	input wire [31:0] i_s7_rdata,
	output bit [31:0] o_s7_wdata,
	output bit [3:0] o_s7_wmask,

	// Slave 8
	output bit o_s8_rw,
	output bit o_s8_request,
	input wire i_s8_ready,
	output bit [31:0] o_s8_address,
	input wire [31:0] i_s8_rdata,
	output bit [31:0] o_s8_wdata,
	output bit [3:0] o_s8_wmask,

	// Slave 9
	output bit o_s9_rw,
	output bit o_s9_request,
	input wire i_s9_ready,
	output bit [31:0] o_s9_address,
	input wire [31:0] i_s9_rdata,
	output bit [31:0] o_s9_wdata,
	output bit [3:0] o_s9_wmask,

	// Slave 10
	output bit o_s10_rw,
	output bit o_s10_request,
	input wire i_s10_ready,
	output bit [31:0] o_s10_address,
	input wire [31:0] i_s10_rdata,
	output bit [31:0] o_s10_wdata,
	output bit [3:0] o_s10_wmask,

	// Slave 11
	output bit o_s11_rw,
	output bit o_s11_request,
	input wire i_s11_ready,
	output bit [31:0] o_s11_address,
	input wire [31:0] i_s11_rdata,
	output bit [31:0] o_s11_wdata,
	output bit [3:0] o_s11_wmask,

	// Slave 12
	output bit o_s12_rw,
	output bit o_s12_request,
	input wire i_s12_ready,
	output bit [31:0] o_s12_address,
	input wire [31:0] i_s12_rdata,
	output bit [31:0] o_s12_wdata,
	output bit [3:0] o_s12_wmask
);
	bit [3:0] s0_source;
	bit [3:0] next_s0_source;
	bit [3:0] s1_source;
	bit [3:0] next_s1_source;
	bit [3:0] s2_source;
	bit [3:0] next_s2_source;
	bit [3:0] s3_source;
	bit [3:0] next_s3_source;
	bit [3:0] s4_source;
	bit [3:0] next_s4_source;
	bit [3:0] s5_source;
	bit [3:0] next_s5_source;
	bit [3:0] s6_source;
	bit [3:0] next_s6_source;
	bit [3:0] s7_source;
	bit [3:0] next_s7_source;
	bit [3:0] s8_source;
	bit [3:0] next_s8_source;
	bit [3:0] s9_source;
	bit [3:0] next_s9_source;
	bit [3:0] s10_source;
	bit [3:0] next_s10_source;
	bit [3:0] s11_source;
	bit [3:0] next_s11_source;
	bit [3:0] s12_source;
	bit [3:0] next_s12_source;

	always_ff @(posedge i_clock) begin
		s0_source <= next_s0_source;
		s1_source <= next_s1_source;
		s2_source <= next_s2_source;
		s3_source <= next_s3_source;
		s4_source <= next_s4_source;
		s5_source <= next_s5_source;
		s6_source <= next_s6_source;
		s7_source <= next_s7_source;
		s8_source <= next_s8_source;
		s9_source <= next_s9_source;
		s10_source <= next_s10_source;
		s11_source <= next_s11_source;
		s12_source <= next_s12_source;
	end

	always_comb begin
		o_s0_rw = 1'b0;
		o_s0_request = 1'b0;
		o_s0_address = 32'h0;
		o_s0_wdata = 32'h0;
		o_s0_wmask = 4'h0;
		o_s1_rw = 1'b0;
		o_s1_request = 1'b0;
		o_s1_address = 32'h0;
		o_s1_wdata = 32'h0;
		o_s1_wmask = 4'h0;
		o_s2_rw = 1'b0;
		o_s2_request = 1'b0;
		o_s2_address = 32'h0;
		o_s2_wdata = 32'h0;
		o_s2_wmask = 4'h0;
		o_s3_rw = 1'b0;
		o_s3_request = 1'b0;
		o_s3_address = 32'h0;
		o_s3_wdata = 32'h0;
		o_s3_wmask = 4'h0;
		o_s4_rw = 1'b0;
		o_s4_request = 1'b0;
		o_s4_address = 32'h0;
		o_s4_wdata = 32'h0;
		o_s4_wmask = 4'h0;
		o_s5_rw = 1'b0;
		o_s5_request = 1'b0;
		o_s5_address = 32'h0;
		o_s5_wdata = 32'h0;
		o_s5_wmask = 4'h0;
		o_s6_rw = 1'b0;
		o_s6_request = 1'b0;
		o_s6_address = 32'h0;
		o_s6_wdata = 32'h0;
		o_s6_wmask = 4'h0;
		o_s7_rw = 1'b0;
		o_s7_request = 1'b0;
		o_s7_address = 32'h0;
		o_s7_wdata = 32'h0;
		o_s7_wmask = 4'h0;
		o_s8_rw = 1'b0;
		o_s8_request = 1'b0;
		o_s8_address = 32'h0;
		o_s8_wdata = 32'h0;
		o_s8_wmask = 4'h0;
		o_s9_rw = 1'b0;
		o_s9_request = 1'b0;
		o_s9_address = 32'h0;
		o_s9_wdata = 32'h0;
		o_s9_wmask = 4'h0;
		o_s10_rw = 1'b0;
		o_s10_request = 1'b0;
		o_s10_address = 32'h0;
		o_s10_wdata = 32'h0;
		o_s10_wmask = 4'h0;
		o_s11_rw = 1'b0;
		o_s11_request = 1'b0;
		o_s11_address = 32'h0;
		o_s11_wdata = 32'h0;
		o_s11_wmask = 4'h0;
		o_s12_rw = 1'b0;
		o_s12_request = 1'b0;
		o_s12_address = 32'h0;
		o_s12_wdata = 32'h0;
		o_s12_wmask = 4'h0;

		o_m0_ready = 1'b0;
		o_m0_rdata = 32'h0;
		o_m1_ready = 1'b0;
		o_m1_rdata = 32'h0;
		o_m2_ready = 1'b0;
		o_m2_rdata = 32'h0;
		o_m3_ready = 1'b0;
		o_m3_rdata = 32'h0;

		if (next_s0_source == 4'h1) begin
			o_s0_rw = i_m0_rw;
			o_s0_request = 1'b1;
			o_m0_ready = i_s0_ready;
			o_s0_address = { 4'h0, i_m0_address[27:0] };
			o_m0_rdata = i_s0_rdata;
			o_s0_wdata = i_m0_wdata;
			o_s0_wmask = i_m0_wmask;
		end
		else if (next_s0_source == 4'h2) begin
			o_s0_rw = i_m1_rw;
			o_s0_request = 1'b1;
			o_m1_ready = i_s0_ready;
			o_s0_address = { 4'h0, i_m1_address[27:0] };
			o_m1_rdata = i_s0_rdata;
			o_s0_wdata = i_m1_wdata;
			o_s0_wmask = i_m1_wmask;
		end
		else if (next_s0_source == 4'h3) begin
			o_s0_rw = i_m2_rw;
			o_s0_request = 1'b1;
			o_m2_ready = i_s0_ready;
			o_s0_address = { 4'h0, i_m2_address[27:0] };
			o_m2_rdata = i_s0_rdata;
			o_s0_wdata = i_m2_wdata;
			o_s0_wmask = i_m2_wmask;
		end
		else if (next_s0_source == 4'h4) begin
			o_s0_rw = i_m3_rw;
			o_s0_request = 1'b1;
			o_m3_ready = i_s0_ready;
			o_s0_address = { 4'h0, i_m3_address[27:0] };
			o_m3_rdata = i_s0_rdata;
			o_s0_wdata = i_m3_wdata;
			o_s0_wmask = i_m3_wmask;
		end

		if (next_s1_source == 4'h1) begin
			o_s1_rw = i_m0_rw;
			o_s1_request = 1'b1;
			o_m0_ready = i_s1_ready;
			o_s1_address = { 4'h0, i_m0_address[27:0] };
			o_m0_rdata = i_s1_rdata;
			o_s1_wdata = i_m0_wdata;
			o_s1_wmask = i_m0_wmask;
		end
		else if (next_s1_source == 4'h2) begin
			o_s1_rw = i_m1_rw;
			o_s1_request = 1'b1;
			o_m1_ready = i_s1_ready;
			o_s1_address = { 4'h0, i_m1_address[27:0] };
			o_m1_rdata = i_s1_rdata;
			o_s1_wdata = i_m1_wdata;
			o_s1_wmask = i_m1_wmask;
		end
		else if (next_s1_source == 4'h3) begin
			o_s1_rw = i_m2_rw;
			o_s1_request = 1'b1;
			o_m2_ready = i_s1_ready;
			o_s1_address = { 4'h0, i_m2_address[27:0] };
			o_m2_rdata = i_s1_rdata;
			o_s1_wdata = i_m2_wdata;
			o_s1_wmask = i_m2_wmask;
		end
		else if (next_s1_source == 4'h4) begin
			o_s1_rw = i_m3_rw;
			o_s1_request = 1'b1;
			o_m3_ready = i_s1_ready;
			o_s1_address = { 4'h0, i_m3_address[27:0] };
			o_m3_rdata = i_s1_rdata;
			o_s1_wdata = i_m3_wdata;
			o_s1_wmask = i_m3_wmask;
		end

		if (next_s2_source == 4'h1) begin
			o_s2_rw = i_m0_rw;
			o_s2_request = 1'b1;
			o_m0_ready = i_s2_ready;
			o_s2_address = { 4'h0, i_m0_address[27:0] };
			o_m0_rdata = i_s2_rdata;
			o_s2_wdata = i_m0_wdata;
			o_s2_wmask = i_m0_wmask;
		end
		else if (next_s2_source == 4'h2) begin
			o_s2_rw = i_m1_rw;
			o_s2_request = 1'b1;
			o_m1_ready = i_s2_ready;
			o_s2_address = { 4'h0, i_m1_address[27:0] };
			o_m1_rdata = i_s2_rdata;
			o_s2_wdata = i_m1_wdata;
			o_s2_wmask = i_m1_wmask;
		end
		else if (next_s2_source == 4'h3) begin
			o_s2_rw = i_m2_rw;
			o_s2_request = 1'b1;
			o_m2_ready = i_s2_ready;
			o_s2_address = { 4'h0, i_m2_address[27:0] };
			o_m2_rdata = i_s2_rdata;
			o_s2_wdata = i_m2_wdata;
			o_s2_wmask = i_m2_wmask;
		end
		else if (next_s2_source == 4'h4) begin
			o_s2_rw = i_m3_rw;
			o_s2_request = 1'b1;
			o_m3_ready = i_s2_ready;
			o_s2_address = { 4'h0, i_m3_address[27:0] };
			o_m3_rdata = i_s2_rdata;
			o_s2_wdata = i_m3_wdata;
			o_s2_wmask = i_m3_wmask;
		end

		if (next_s3_source == 4'h1) begin
			o_s3_rw = i_m0_rw;
			o_s3_request = 1'b1;
			o_m0_ready = i_s3_ready;
			o_s3_address = { 4'h0, i_m0_address[27:0] };
			o_m0_rdata = i_s3_rdata;
			o_s3_wdata = i_m0_wdata;
			o_s3_wmask = i_m0_wmask;
		end
		else if (next_s3_source == 4'h2) begin
			o_s3_rw = i_m1_rw;
			o_s3_request = 1'b1;
			o_m1_ready = i_s3_ready;
			o_s3_address = { 4'h0, i_m1_address[27:0] };
			o_m1_rdata = i_s3_rdata;
			o_s3_wdata = i_m1_wdata;
			o_s3_wmask = i_m1_wmask;
		end
		else if (next_s3_source == 4'h3) begin
			o_s3_rw = i_m2_rw;
			o_s3_request = 1'b1;
			o_m2_ready = i_s3_ready;
			o_s3_address = { 4'h0, i_m2_address[27:0] };
			o_m2_rdata = i_s3_rdata;
			o_s3_wdata = i_m2_wdata;
			o_s3_wmask = i_m2_wmask;
		end
		else if (next_s3_source == 4'h4) begin
			o_s3_rw = i_m3_rw;
			o_s3_request = 1'b1;
			o_m3_ready = i_s3_ready;
			o_s3_address = { 4'h0, i_m3_address[27:0] };
			o_m3_rdata = i_s3_rdata;
			o_s3_wdata = i_m3_wdata;
			o_s3_wmask = i_m3_wmask;
		end

		if (next_s4_source == 4'h1) begin
			o_s4_rw = i_m0_rw;
			o_s4_request = 1'b1;
			o_m0_ready = i_s4_ready;
			o_s4_address = { 4'h0, i_m0_address[27:0] };
			o_m0_rdata = i_s4_rdata;
			o_s4_wdata = i_m0_wdata;
			o_s4_wmask = i_m0_wmask;
		end
		else if (next_s4_source == 4'h2) begin
			o_s4_rw = i_m1_rw;
			o_s4_request = 1'b1;
			o_m1_ready = i_s4_ready;
			o_s4_address = { 4'h0, i_m1_address[27:0] };
			o_m1_rdata = i_s4_rdata;
			o_s4_wdata = i_m1_wdata;
			o_s4_wmask = i_m1_wmask;
		end
		else if (next_s4_source == 4'h3) begin
			o_s4_rw = i_m2_rw;
			o_s4_request = 1'b1;
			o_m2_ready = i_s4_ready;
			o_s4_address = { 4'h0, i_m2_address[27:0] };
			o_m2_rdata = i_s4_rdata;
			o_s4_wdata = i_m2_wdata;
			o_s4_wmask = i_m2_wmask;
		end
		else if (next_s4_source == 4'h4) begin
			o_s4_rw = i_m3_rw;
			o_s4_request = 1'b1;
			o_m3_ready = i_s4_ready;
			o_s4_address = { 4'h0, i_m3_address[27:0] };
			o_m3_rdata = i_s4_rdata;
			o_s4_wdata = i_m3_wdata;
			o_s4_wmask = i_m3_wmask;
		end

		if (next_s5_source == 4'h1) begin
			o_s5_rw = i_m0_rw;
			o_s5_request = 1'b1;
			o_m0_ready = i_s5_ready;
			o_s5_address = { 4'h0, i_m0_address[27:0] };
			o_m0_rdata = i_s5_rdata;
			o_s5_wdata = i_m0_wdata;
			o_s5_wmask = i_m0_wmask;
		end
		else if (next_s5_source == 4'h2) begin
			o_s5_rw = i_m1_rw;
			o_s5_request = 1'b1;
			o_m1_ready = i_s5_ready;
			o_s5_address = { 4'h0, i_m1_address[27:0] };
			o_m1_rdata = i_s5_rdata;
			o_s5_wdata = i_m1_wdata;
			o_s5_wmask = i_m1_wmask;
		end
		else if (next_s5_source == 4'h3) begin
			o_s5_rw = i_m2_rw;
			o_s5_request = 1'b1;
			o_m2_ready = i_s5_ready;
			o_s5_address = { 4'h0, i_m2_address[27:0] };
			o_m2_rdata = i_s5_rdata;
			o_s5_wdata = i_m2_wdata;
			o_s5_wmask = i_m2_wmask;
		end
		else if (next_s5_source == 4'h4) begin
			o_s5_rw = i_m3_rw;
			o_s5_request = 1'b1;
			o_m3_ready = i_s5_ready;
			o_s5_address = { 4'h0, i_m3_address[27:0] };
			o_m3_rdata = i_s5_rdata;
			o_s5_wdata = i_m3_wdata;
			o_s5_wmask = i_m3_wmask;
		end

		if (next_s6_source == 4'h1) begin
			o_s6_rw = i_m0_rw;
			o_s6_request = 1'b1;
			o_m0_ready = i_s6_ready;
			o_s6_address = { 4'h0, i_m0_address[27:0] };
			o_m0_rdata = i_s6_rdata;
			o_s6_wdata = i_m0_wdata;
			o_s6_wmask = i_m0_wmask;
		end
		else if (next_s6_source == 4'h2) begin
			o_s6_rw = i_m1_rw;
			o_s6_request = 1'b1;
			o_m1_ready = i_s6_ready;
			o_s6_address = { 4'h0, i_m1_address[27:0] };
			o_m1_rdata = i_s6_rdata;
			o_s6_wdata = i_m1_wdata;
			o_s6_wmask = i_m1_wmask;
		end
		else if (next_s6_source == 4'h3) begin
			o_s6_rw = i_m2_rw;
			o_s6_request = 1'b1;
			o_m2_ready = i_s6_ready;
			o_s6_address = { 4'h0, i_m2_address[27:0] };
			o_m2_rdata = i_s6_rdata;
			o_s6_wdata = i_m2_wdata;
			o_s6_wmask = i_m2_wmask;
		end
		else if (next_s6_source == 4'h4) begin
			o_s6_rw = i_m3_rw;
			o_s6_request = 1'b1;
			o_m3_ready = i_s6_ready;
			o_s6_address = { 4'h0, i_m3_address[27:0] };
			o_m3_rdata = i_s6_rdata;
			o_s6_wdata = i_m3_wdata;
			o_s6_wmask = i_m3_wmask;
		end

		if (next_s7_source == 4'h1) begin
			o_s7_rw = i_m0_rw;
			o_s7_request = 1'b1;
			o_m0_ready = i_s7_ready;
			o_s7_address = { 4'h0, i_m0_address[27:0] };
			o_m0_rdata = i_s7_rdata;
			o_s7_wdata = i_m0_wdata;
			o_s7_wmask = i_m0_wmask;
		end
		else if (next_s7_source == 4'h2) begin
			o_s7_rw = i_m1_rw;
			o_s7_request = 1'b1;
			o_m1_ready = i_s7_ready;
			o_s7_address = { 4'h0, i_m1_address[27:0] };
			o_m1_rdata = i_s7_rdata;
			o_s7_wdata = i_m1_wdata;
			o_s7_wmask = i_m1_wmask;
		end
		else if (next_s7_source == 4'h3) begin
			o_s7_rw = i_m2_rw;
			o_s7_request = 1'b1;
			o_m2_ready = i_s7_ready;
			o_s7_address = { 4'h0, i_m2_address[27:0] };
			o_m2_rdata = i_s7_rdata;
			o_s7_wdata = i_m2_wdata;
			o_s7_wmask = i_m2_wmask;
		end
		else if (next_s7_source == 4'h4) begin
			o_s7_rw = i_m3_rw;
			o_s7_request = 1'b1;
			o_m3_ready = i_s7_ready;
			o_s7_address = { 4'h0, i_m3_address[27:0] };
			o_m3_rdata = i_s7_rdata;
			o_s7_wdata = i_m3_wdata;
			o_s7_wmask = i_m3_wmask;
		end

		if (next_s8_source == 4'h1) begin
			o_s8_rw = i_m0_rw;
			o_s8_request = 1'b1;
			o_m0_ready = i_s8_ready;
			o_s8_address = { 4'h0, i_m0_address[27:0] };
			o_m0_rdata = i_s8_rdata;
			o_s8_wdata = i_m0_wdata;
			o_s8_wmask = i_m0_wmask;
		end
		else if (next_s8_source == 4'h2) begin
			o_s8_rw = i_m1_rw;
			o_s8_request = 1'b1;
			o_m1_ready = i_s8_ready;
			o_s8_address = { 4'h0, i_m1_address[27:0] };
			o_m1_rdata = i_s8_rdata;
			o_s8_wdata = i_m1_wdata;
			o_s8_wmask = i_m1_wmask;
		end
		else if (next_s8_source == 4'h3) begin
			o_s8_rw = i_m2_rw;
			o_s8_request = 1'b1;
			o_m2_ready = i_s8_ready;
			o_s8_address = { 4'h0, i_m2_address[27:0] };
			o_m2_rdata = i_s8_rdata;
			o_s8_wdata = i_m2_wdata;
			o_s8_wmask = i_m2_wmask;
		end
		else if (next_s8_source == 4'h4) begin
			o_s8_rw = i_m3_rw;
			o_s8_request = 1'b1;
			o_m3_ready = i_s8_ready;
			o_s8_address = { 4'h0, i_m3_address[27:0] };
			o_m3_rdata = i_s8_rdata;
			o_s8_wdata = i_m3_wdata;
			o_s8_wmask = i_m3_wmask;
		end

		if (next_s9_source == 4'h1) begin
			o_s9_rw = i_m0_rw;
			o_s9_request = 1'b1;
			o_m0_ready = i_s9_ready;
			o_s9_address = { 4'h0, i_m0_address[27:0] };
			o_m0_rdata = i_s9_rdata;
			o_s9_wdata = i_m0_wdata;
			o_s9_wmask = i_m0_wmask;
		end
		else if (next_s9_source == 4'h2) begin
			o_s9_rw = i_m1_rw;
			o_s9_request = 1'b1;
			o_m1_ready = i_s9_ready;
			o_s9_address = { 4'h0, i_m1_address[27:0] };
			o_m1_rdata = i_s9_rdata;
			o_s9_wdata = i_m1_wdata;
			o_s9_wmask = i_m1_wmask;
		end
		else if (next_s9_source == 4'h3) begin
			o_s9_rw = i_m2_rw;
			o_s9_request = 1'b1;
			o_m2_ready = i_s9_ready;
			o_s9_address = { 4'h0, i_m2_address[27:0] };
			o_m2_rdata = i_s9_rdata;
			o_s9_wdata = i_m2_wdata;
			o_s9_wmask = i_m2_wmask;
		end
		else if (next_s9_source == 4'h4) begin
			o_s9_rw = i_m3_rw;
			o_s9_request = 1'b1;
			o_m3_ready = i_s9_ready;
			o_s9_address = { 4'h0, i_m3_address[27:0] };
			o_m3_rdata = i_s9_rdata;
			o_s9_wdata = i_m3_wdata;
			o_s9_wmask = i_m3_wmask;
		end

		if (next_s10_source == 4'h1) begin
			o_s10_rw = i_m0_rw;
			o_s10_request = 1'b1;
			o_m0_ready = i_s10_ready;
			o_s10_address = { 4'h0, i_m0_address[27:0] };
			o_m0_rdata = i_s10_rdata;
			o_s10_wdata = i_m0_wdata;
			o_s10_wmask = i_m0_wmask;
		end
		else if (next_s10_source == 4'h2) begin
			o_s10_rw = i_m1_rw;
			o_s10_request = 1'b1;
			o_m1_ready = i_s10_ready;
			o_s10_address = { 4'h0, i_m1_address[27:0] };
			o_m1_rdata = i_s10_rdata;
			o_s10_wdata = i_m1_wdata;
			o_s10_wmask = i_m1_wmask;
		end
		else if (next_s10_source == 4'h3) begin
			o_s10_rw = i_m2_rw;
			o_s10_request = 1'b1;
			o_m2_ready = i_s10_ready;
			o_s10_address = { 4'h0, i_m2_address[27:0] };
			o_m2_rdata = i_s10_rdata;
			o_s10_wdata = i_m2_wdata;
			o_s10_wmask = i_m2_wmask;
		end
		else if (next_s10_source == 4'h4) begin
			o_s10_rw = i_m3_rw;
			o_s10_request = 1'b1;
			o_m3_ready = i_s10_ready;
			o_s10_address = { 4'h0, i_m3_address[27:0] };
			o_m3_rdata = i_s10_rdata;
			o_s10_wdata = i_m3_wdata;
			o_s10_wmask = i_m3_wmask;
		end

		if (next_s11_source == 4'h1) begin
			o_s11_rw = i_m0_rw;
			o_s11_request = 1'b1;
			o_m0_ready = i_s11_ready;
			o_s11_address = { 4'h0, i_m0_address[27:0] };
			o_m0_rdata = i_s11_rdata;
			o_s11_wdata = i_m0_wdata;
			o_s11_wmask = i_m0_wmask;
		end
		else if (next_s11_source == 4'h2) begin
			o_s11_rw = i_m1_rw;
			o_s11_request = 1'b1;
			o_m1_ready = i_s11_ready;
			o_s11_address = { 4'h0, i_m1_address[27:0] };
			o_m1_rdata = i_s11_rdata;
			o_s11_wdata = i_m1_wdata;
			o_s11_wmask = i_m1_wmask;
		end
		else if (next_s11_source == 4'h3) begin
			o_s11_rw = i_m2_rw;
			o_s11_request = 1'b1;
			o_m2_ready = i_s11_ready;
			o_s11_address = { 4'h0, i_m2_address[27:0] };
			o_m2_rdata = i_s11_rdata;
			o_s11_wdata = i_m2_wdata;
			o_s11_wmask = i_m2_wmask;
		end
		else if (next_s11_source == 4'h4) begin
			o_s11_rw = i_m3_rw;
			o_s11_request = 1'b1;
			o_m3_ready = i_s11_ready;
			o_s11_address = { 4'h0, i_m3_address[27:0] };
			o_m3_rdata = i_s11_rdata;
			o_s11_wdata = i_m3_wdata;
			o_s11_wmask = i_m3_wmask;
		end

		if (next_s12_source == 4'h1) begin
			o_s12_rw = i_m0_rw;
			o_s12_request = 1'b1;
			o_m0_ready = i_s12_ready;
			o_s12_address = { 4'h0, i_m0_address[27:0] };
			o_m0_rdata = i_s12_rdata;
			o_s12_wdata = i_m0_wdata;
			o_s12_wmask = i_m0_wmask;
		end
		else if (next_s12_source == 4'h2) begin
			o_s12_rw = i_m1_rw;
			o_s12_request = 1'b1;
			o_m1_ready = i_s12_ready;
			o_s12_address = { 4'h0, i_m1_address[27:0] };
			o_m1_rdata = i_s12_rdata;
			o_s12_wdata = i_m1_wdata;
			o_s12_wmask = i_m1_wmask;
		end
		else if (next_s12_source == 4'h3) begin
			o_s12_rw = i_m2_rw;
			o_s12_request = 1'b1;
			o_m2_ready = i_s12_ready;
			o_s12_address = { 4'h0, i_m2_address[27:0] };
			o_m2_rdata = i_s12_rdata;
			o_s12_wdata = i_m2_wdata;
			o_s12_wmask = i_m2_wmask;
		end
		else if (next_s12_source == 4'h4) begin
			o_s12_rw = i_m3_rw;
			o_s12_request = 1'b1;
			o_m3_ready = i_s12_ready;
			o_s12_address = { 4'h0, i_m3_address[27:0] };
			o_m3_rdata = i_s12_rdata;
			o_s12_wdata = i_m3_wdata;
			o_s12_wmask = i_m3_wmask;
		end
	end

	always_comb begin
		next_s0_source = 4'h0;
		next_s1_source = 4'h0;
		next_s2_source = 4'h0;
		next_s3_source = 4'h0;
		next_s4_source = 4'h0;
		next_s5_source = 4'h0;
		next_s6_source = 4'h0;
		next_s7_source = 4'h0;
		next_s8_source = 4'h0;
		next_s9_source = 4'h0;
		next_s10_source = 4'h0;
		next_s11_source = 4'h0;
		next_s12_source = 4'h0;

		if (i_m3_request) begin
			if (
				i_m3_address[31:28] == 4'h0 &&
				(s0_source == 4'h0 || s0_source == 4'h4)
			) begin
				next_s0_source = 4'h4;
			end
			if (
				i_m3_address[31:28] == 4'h1 &&
				(s1_source == 4'h0 || s1_source == 4'h4)
			) begin
				next_s1_source = 4'h4;
			end
			if (
				i_m3_address[31:28] == 4'h2 &&
				(s2_source == 4'h0 || s2_source == 4'h4)
			) begin
				next_s2_source = 4'h4;
			end
			if (
				i_m3_address[31:28] == 4'h3 &&
				(s3_source == 4'h0 || s3_source == 4'h4)
			) begin
				next_s3_source = 4'h4;
			end
			if (
				i_m3_address[31:28] == 4'h4 &&
				(s4_source == 4'h0 || s4_source == 4'h4)
			) begin
				next_s4_source = 4'h4;
			end
			if (
				i_m3_address[31:28] == 4'h5 &&
				(s5_source == 4'h0 || s5_source == 4'h4)
			) begin
				next_s5_source = 4'h4;
			end
			if (
				i_m3_address[31:28] == 4'h6 &&
				(s6_source == 4'h0 || s6_source == 4'h4)
			) begin
				next_s6_source = 4'h4;
			end
			if (
				i_m3_address[31:28] == 4'h7 &&
				(s7_source == 4'h0 || s7_source == 4'h4)
			) begin
				next_s7_source = 4'h4;
			end
			if (
				i_m3_address[31:28] == 4'h8 &&
				(s8_source == 4'h0 || s8_source == 4'h4)
			) begin
				next_s8_source = 4'h4;
			end
			if (
				i_m3_address[31:28] == 4'h9 &&
				(s9_source == 4'h0 || s9_source == 4'h4)
			) begin
				next_s9_source = 4'h4;
			end
			if (
				i_m3_address[31:28] == 4'ha &&
				(s10_source == 4'h0 || s10_source == 4'h4)
			) begin
				next_s10_source = 4'h4;
			end
			if (
				i_m3_address[31:28] == 4'hb &&
				(s11_source == 4'h0 || s11_source == 4'h4)
			) begin
				next_s11_source = 4'h4;
			end
			if (
				i_m3_address[31:28] == 4'hc &&
				(s12_source == 4'h0 || s12_source == 4'h4)
			) begin
				next_s12_source = 4'h4;
			end
		end

		if (i_m2_request) begin
			if (
				i_m2_address[31:28] == 4'h0 &&
				(s0_source == 4'h0 || s0_source == 4'h3)
			) begin
				next_s0_source = 4'h3;
			end
			if (
				i_m2_address[31:28] == 4'h1 &&
				(s1_source == 4'h0 || s1_source == 4'h3)
			) begin
				next_s1_source = 4'h3;
			end
			if (
				i_m2_address[31:28] == 4'h2 &&
				(s2_source == 4'h0 || s2_source == 4'h3)
			) begin
				next_s2_source = 4'h3;
			end
			if (
				i_m2_address[31:28] == 4'h3 &&
				(s3_source == 4'h0 || s3_source == 4'h3)
			) begin
				next_s3_source = 4'h3;
			end
			if (
				i_m2_address[31:28] == 4'h4 &&
				(s4_source == 4'h0 || s4_source == 4'h3)
			) begin
				next_s4_source = 4'h3;
			end
			if (
				i_m2_address[31:28] == 4'h5 &&
				(s5_source == 4'h0 || s5_source == 4'h3)
			) begin
				next_s5_source = 4'h3;
			end
			if (
				i_m2_address[31:28] == 4'h6 &&
				(s6_source == 4'h0 || s6_source == 4'h3)
			) begin
				next_s6_source = 4'h3;
			end
			if (
				i_m2_address[31:28] == 4'h7 &&
				(s7_source == 4'h0 || s7_source == 4'h3)
			) begin
				next_s7_source = 4'h3;
			end
			if (
				i_m2_address[31:28] == 4'h8 &&
				(s8_source == 4'h0 || s8_source == 4'h3)
			) begin
				next_s8_source = 4'h3;
			end
			if (
				i_m2_address[31:28] == 4'h9 &&
				(s9_source == 4'h0 || s9_source == 4'h3)
			) begin
				next_s9_source = 4'h3;
			end
			if (
				i_m2_address[31:28] == 4'ha &&
				(s10_source == 4'h0 || s10_source == 4'h3)
			) begin
				next_s10_source = 4'h3;
			end
			if (
				i_m2_address[31:28] == 4'hb &&
				(s11_source == 4'h0 || s11_source == 4'h3)
			) begin
				next_s11_source = 4'h3;
			end
			if (
				i_m2_address[31:28] == 4'hc &&
				(s12_source == 4'h0 || s12_source == 4'h3)
			) begin
				next_s12_source = 4'h3;
			end
		end

		if (i_m1_request) begin
			if (
				i_m1_address[31:28] == 4'h0 &&
				(s0_source == 4'h0 || s0_source == 4'h2)
			) begin
				next_s0_source = 4'h2;
			end
			if (
				i_m1_address[31:28] == 4'h1 &&
				(s1_source == 4'h0 || s1_source == 4'h2)
			) begin
				next_s1_source = 4'h2;
			end
			if (
				i_m1_address[31:28] == 4'h2 &&
				(s2_source == 4'h0 || s2_source == 4'h2)
			) begin
				next_s2_source = 4'h2;
			end
			if (
				i_m1_address[31:28] == 4'h3 &&
				(s3_source == 4'h0 || s3_source == 4'h2)
			) begin
				next_s3_source = 4'h2;
			end
			if (
				i_m1_address[31:28] == 4'h4 &&
				(s4_source == 4'h0 || s4_source == 4'h2)
			) begin
				next_s4_source = 4'h2;
			end
			if (
				i_m1_address[31:28] == 4'h5 &&
				(s5_source == 4'h0 || s5_source == 4'h2)
			) begin
				next_s5_source = 4'h2;
			end
			if (
				i_m1_address[31:28] == 4'h6 &&
				(s6_source == 4'h0 || s6_source == 4'h2)
			) begin
				next_s6_source = 4'h2;
			end
			if (
				i_m1_address[31:28] == 4'h7 &&
				(s7_source == 4'h0 || s7_source == 4'h2)
			) begin
				next_s7_source = 4'h2;
			end
			if (
				i_m1_address[31:28] == 4'h8 &&
				(s8_source == 4'h0 || s8_source == 4'h2)
			) begin
				next_s8_source = 4'h2;
			end
			if (
				i_m1_address[31:28] == 4'h9 &&
				(s9_source == 4'h0 || s9_source == 4'h2)
			) begin
				next_s9_source = 4'h2;
			end
			if (
				i_m1_address[31:28] == 4'ha &&
				(s10_source == 4'h0 || s10_source == 4'h2)
			) begin
				next_s10_source = 4'h2;
			end
			if (
				i_m1_address[31:28] == 4'hb &&
				(s11_source == 4'h0 || s11_source == 4'h2)
			) begin
				next_s11_source = 4'h2;
			end
			if (
				i_m1_address[31:28] == 4'hc &&
				(s12_source == 4'h0 || s12_source == 4'h2)
			) begin
				next_s12_source = 4'h2;
			end
		end

		if (i_m0_request) begin
			if (
				i_m0_address[31:28] == 4'h0 &&
				(s0_source == 4'h0 || s0_source == 4'h1)
			) begin
				next_s0_source = 4'h1;
			end
			if (
				i_m0_address[31:28] == 4'h1 &&
				(s1_source == 4'h0 || s1_source == 4'h1)
			) begin
				next_s1_source = 4'h1;
			end
			if (
				i_m0_address[31:28] == 4'h2 &&
				(s2_source == 4'h0 || s2_source == 4'h1)
			) begin
				next_s2_source = 4'h1;
			end
			if (
				i_m0_address[31:28] == 4'h3 &&
				(s3_source == 4'h0 || s3_source == 4'h1)
			) begin
				next_s3_source = 4'h1;
			end
			if (
				i_m0_address[31:28] == 4'h4 &&
				(s4_source == 4'h0 || s4_source == 4'h1)
			) begin
				next_s4_source = 4'h1;
			end
			if (
				i_m0_address[31:28] == 4'h5 &&
				(s5_source == 4'h0 || s5_source == 4'h1)
			) begin
				next_s5_source = 4'h1;
			end
			if (
				i_m0_address[31:28] == 4'h6 &&
				(s6_source == 4'h0 || s6_source == 4'h1)
			) begin
				next_s6_source = 4'h1;
			end
			if (
				i_m0_address[31:28] == 4'h7 &&
				(s7_source == 4'h0 || s7_source == 4'h1)
			) begin
				next_s7_source = 4'h1;
			end
			if (
				i_m0_address[31:28] == 4'h8 &&
				(s8_source == 4'h0 || s8_source == 4'h1)
			) begin
				next_s8_source = 4'h1;
			end
			if (
				i_m0_address[31:28] == 4'h9 &&
				(s9_source == 4'h0 || s9_source == 4'h1)
			) begin
				next_s9_source = 4'h1;
			end
			if (
				i_m0_address[31:28] == 4'ha &&
				(s10_source == 4'h0 || s10_source == 4'h1)
			) begin
				next_s10_source = 4'h1;
			end
			if (
				i_m0_address[31:28] == 4'hb &&
				(s11_source == 4'h0 || s11_source == 4'h1)
			) begin
				next_s11_source = 4'h1;
			end
			if (
				i_m0_address[31:28] == 4'hc &&
				(s12_source == 4'h0 || s12_source == 4'h1)
			) begin
				next_s12_source = 4'h1;
			end
		end
	end
endmodule
