...
{
	// I
	if ((word & 0x0000707f) == 0x00005003)	// LHU
	{
...
{
	// R
	if ((word & 0xfe00707f) == 0x00005033)	// SRL
	{
		const auto f = FormatR::parse(word);
	...
	// R
	else if ((word & 0xfe00707f) == 0x60001033)	// ROL
	{
		const auto f = FormatR::parse(word);
		const uint32_t shamt = R_u(f.rs2);
		R_u(f.rd) = (R_u(f.rs1) << shamt) | (R_u(f.rs1) >> (32 - shamt));
...
{
	// R
	if ((word & 0xfe00707f) == 0x2000203b)	// SH1ADDUW
	{
		const auto f = FormatR::parse(word);
...
{
	// R
	if ((word & 0xfe00707f) == 0xa0000053)	// FLE
	{
		const auto f = FormatR::parse(word);
